...
-- Les references necessaires sont introduites dans un fichier du repertoire
   Articles/ dont le nom verifie les conditions suivantes :
   -- suffixe .html
   -- ne commence pas par .
...
<script type="text/javascript">bibref("Wedeniwski*02")</script>
   ...
   GlobalBiblioFile/Global.bib

-- Le script perl Biblio/UpDateBiblio.pl doit etre lance a la
   main depuis le repertoire Biblio/
   ...
   Biblio/ExceptionalRefs.txt sert a gerer ces exceptions.
...
-- Ce fichier Biblio/ExceptionalRefs.txt contient des entrees au
   format 
...
-- The required references are introduced in a file of the directory
   Articles/ whose name verifies the following conditions:
   -- suffix is .html
   -- doesn't start by .
...
-- The Perl script Biblio/UpDateBiblio.pl has to be run from
   the Biblio/ directory to regenerate
   the web pages from the directory Biblio/. This script will
   ...
   Biblio/ExceptionalRefs.txt, see this file for its format.
...
-- The file Biblio/ExceptionalRefs.txt contains entries
