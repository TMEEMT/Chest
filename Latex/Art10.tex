...
\begin{thm}{Theorem (2008)}
  ...
  $(0.1333, 1664)$, by $(1/2, 8)$ or by $(1,1)$.
\end{thm}
...
\par 
  ...
  contains the next evaluation.
  \par 
\begin{thm}{Theorem (1983)}

	For $y \ge 1$, we have
	$$
	\sum_{q\le y}\mu^2(q)\prod_{\substack{p|q\\ p\neq
	2}}\frac{2}{p-2}
	=
	\tfrac14\prod_{p > 2}\frac{(p-1)^2}{p(p-2)}
	\biggl((\log y)^2-A_3\log y - A_4+\mathcal{O}^*(1088/y^{1/3})\biggr)
	$$
	where
	$A_3=6.023476\cdots$ and $A_4=1.114073\cdots$.
\end{thm}
...
\begin{thm}{Theorem (2021)}
  ...
  For $x\ge 2$, we have
$$
\sum_{n\le x} \tau_4(n) = C_1x(\log x)^3
+ C_2x(\log x)^2
+C_3x(\log x
+C_4x
...
x}\tau_4(n)\le(1/3)x(\log x)^3$ when $x\ge 193$.
...
\begin{thm}{Theorem (2021)}
  ...
  For $x\ge 2$, we have
$$
\sum_{n\le x} \tau(n)^2 = D_1x(\log x)^3
+ D_2x(\log x)^2
+D_3x(\log x
+D_4x
...
They deduce for instance from this that $\sum_{n\le
x}\tau(n)^2\le(1/4)x(\log x)^3$ when $x\ge 433$ and that 
$\sum_{n\le
x}\tau(n)^2\le x(\log x)^3$ when $x\ge 7$.
